...
> [!IMPORTANT]
> **SAFETY GUIDELINES**
> - **Local Only**: All memory observations must remain on the local server.
> - **External Sharing**: The `observationFeed` (streaming to Telegram/Discord) must NOT be enabled unless explicitly requested and configured by the user.
> - **No Exfiltration**: Do not send any codebase information or memory data to external URLs.
> - **Privacy**: Do not log sensitive credentials (like those found in `.env` or config files) into the persistent memory.
