...
- `openclaw` — Persistent memory and session continuity for AI agents. Records and recalls observations across development sessions to maintain long-term context. Includes hardened safety guidelines to ensure data stays local.
- `security-review` — Deep semantic security auditing. Uses senior-security-engineer logic to identify high-confidence vulnerabilities (Injection, Auth, Crypto, etc.) in code changes.
